...
//------------------------------------  Функция разбора принятой в Serial port строки

void parseParams(String inputString) {
  String bb = inputString;
  if (bb.substring(0, 2) == "TW") {
    String T1 = bb.substring(bb.indexOf("TW", 2) + 3);
...
void loop(void) {
  //------------------------------------ Serial port, пока не конец сообщения, читаем данные и формируем строку
  while (Serial.available()) {
    char ch = Serial.read();
    serialBuffer += ch;
    if (ch == '\n') {
      parseParams(serialBuffer); //вызываем функцию разбора принятой строки
      serialBuffer = "";
    }
  }
